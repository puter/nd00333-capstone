...
def clean_data(data):
    ...
    x_df["gender"] = x_df.marital.apply(lambda s: 1 if s == "Male" else 0)
    x_df["ever_married"] = x_df.default.apply(lambda s: 1 if s == "Yes" else 0)
    x_df["Residence_type"] = x_df.housing.apply(lambda s: 1 if s == "Urban" else 0)
